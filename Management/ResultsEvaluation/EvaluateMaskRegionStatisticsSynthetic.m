...
AlgorithmNames={'16b'} %'01' '02' '04' '05' '06' '07' '08' '10' '14' '16a' '16b' '17' };

ImageRoot='/media/marzampoglou/3TB_B/ImageForensics/Datasets/';
InputRoot='/media/marzampoglou/3TB_A/AlgorithmOutput/';
OutputRoot='/media/marzampoglou/3TB_A/AlgorithmOutput/Evaluations/';

MaskRoot=[ImageRoot 'Masks/'];

Qualities=  [90] % 100 95 85 75 65];
Rescales= [95 75 50];% [false]; %  

load('../../Datasets_Independent.mat'); %
DatasetList= { 'VIPPDempSchaSynth' 'VIPPDempSchaReal'}; % 'FirstChallengeTrain' 'Carvalho','ColumbiauUncomp',}; %'VIPP2', , 'FirstChallengeTest2',
        ...
        for Rescale=Rescales
            for Dataset=1:length(DatasetList)
                ...
                for subfolder=1:length(InputPaths);
                    InputPath=[InputRoot AlgorithmName '/' num2str(Quality) '_' num2str(Rescale) '/' InputPaths{subfolder}];
                    ...
                    for InputFile=1:length(FileList)
                        ...
                        for MapInd=1:length(ResultMap)
                            ...
                            if isempty(Results(InputFile,MapInd).OutsideHist)
