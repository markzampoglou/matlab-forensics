...
for ii=1:length(List)
    ...
    for X=1:size(R.Report.Curves,1)
       for Y=1:size(R.Report.Curves,2)
            for FieldInd=1:length(Fields)
                if ~strcmp(Fields{FieldInd},'Name')
                    for Vert=AuSp(Y,1):2:size(R.Report.Curves{X,Y}.(Fields{FieldInd}),1)
                        NewSeries=R.Report.Curves{X,Y}.(Fields{FieldInd})([Vert Vert+AuSp(Y,2)],:);
